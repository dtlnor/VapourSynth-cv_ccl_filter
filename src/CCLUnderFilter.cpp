...
static void process_ccl_under(const VSFrame* src, VSFrame* dst, const MaskData* const VS_RESTRICT d, const VSAPI* vsapi) {
...
static const VSFrame* VS_CC excludeCCLUnderGetFrame(int n, int activationReason, void* instanceData, void** frameData, VSFrameContext* frameCtx, VSCore* core, const VSAPI* vsapi) {
	...
	if (activationReason == arInitial) {
		...
		if (d->vi->format.colorFamily == cfGray) {
			process_ccl_under(src, dst, d, vsapi);
		}
...
static void VS_CC excludeCCLUnderMapFree(void* instanceData, VSCore* core, const VSAPI* vsapi) {
	auto d{ static_cast<MaskData*>(instanceData) };
	vsapi->freeNode(d->node);
	delete d;
}

void VS_CC excludeCCLUnderCreate(const VSMap* in, VSMap* out, void* userData, VSCore* core, const VSAPI* vsapi) {
	...
	if (err) {
		vsapi->mapSetError(out, "ExcludeCCLUnder: cc_thr must be specified.");
		vsapi->freeNode(d->node);
		return;
	}
	...
	if (err)
		...
		if (cc_stat_type < 0 || cc_stat_type > 4) {
			vsapi->mapSetError(out, "ExcludeCCLUnder: cc_stat_type must be between 0 and 4.");
			vsapi->freeNode(d->node);
			return;
		}
	...
	if (err)
		...
		if (connectivity != 4 && connectivity != 8) {
			vsapi->mapSetError(out, "ExcludeCCLUnder: connectivity must be 4 or 8.");
			vsapi->freeNode(d->node);
			return;
		}
	...
	if (d->vi->format.bytesPerSample != 1 || (d->vi->format.colorFamily != cfGray)) {
		vsapi->mapSetError(out, "ExcludeCCLUnder: only Gray8 formats supported.");
		vsapi->freeNode(d->node);
		return;
	}

	VSFilterDependency deps[]{ {d->node, rpGeneral} };
	vsapi->createVideoFilter(out, "ExcludeCCLUnder", d->vi, excludeCCLUnderGetFrame, excludeCCLUnderMapFree, fmParallel, deps, 1, d.get(), core);
	d.release();
}